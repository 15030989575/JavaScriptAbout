...
- 我们称之为 **脚本**。它们可以写在 HTML 中，在页面加载的时候会自动执行。

- 脚本作为纯文本存在和执行。它们不需要特殊的准备或编译即可运行。

- 为什么叫 JavaScript？

*JavaScript 在刚诞生的时候，它的名字叫 “LiveScript”。但是因为当时 Java 很流行，所以决定将一种新语言定位为 Java 的结伴语言会有助于它的流行，是一种营销策略。*



- 随着 JavaScript 的发展，它已经变成了一门独立的语言，同时也有了自己的语言规范 [ECMAScript](http://en.wikipedia.org/wiki/ECMAScript)。
...
- 不同的引擎有不同的“代号”，例如：
  -  —— Chrome 和 Opera 中的 JavaScript 引擎。
...
- 有很多其他的语言可以被编译成 JavaScript，这些语言还提供了更多的功能。可以了解一下这些语言
